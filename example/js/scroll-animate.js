 ...
 * 页面元素入场动画
 * 使用方法：
 * 1. 在需要加入动画的元素上加上 class="san [动画class名称]" 即可
 * 2. [动画class名称] 可以是 fadeInLeft、fadeInUp、fadeInRight、zoomIn、slideInLeft等等，也可以自定义动画
 * 3. 同时可加上属性 data-san-delay="0.3s" 用于设置动画入场延迟时间
 */
  ...
  class _MutationObserver {
    constructor() {
      if (typeof console !== 'undefined' && console !== null) {
        ...
        console.warn(
          'cannot detect dom mutations, please call .sync() after loading new content.'
        );
      }
    }
...
class SAN {
  WeakMap = window.WeakMap || window.MozWeakMap ||_WeakMap
  MutationObserver = window.MutationObserver || window.WebkitMutationObserver||_MutationObserver
  getComputedStyle = window.getComputedStyle ||_getComputedStyle
  defaults = {
    boxClass: 'san',
  ...
  // Map of elements to animation names:
    ...
    this.sanEvent = createEvent(this.config.boxClass);
  }
...
// show box element
  ...
  applyStyle(box, hidden) {
    const duration = box.getAttribute('data-san-duration');
    const delay = box.getAttribute('data-san-delay');
    const iteration = box.getAttribute('data-san-iteration');
...
// check if box is visible
  isVisible(box) {
    const offset = box.getAttribute('data-san-offset') || this.config.offset;
    const viewTop = (
...
function scrollAnimateInit(scrollContainer = '#app') {
  const san = document.querySelector('.san')
  if (san) san.style.visibility = 'visible';
  new SAN({
    scrollContainer
  }).init()
}
